            ...
            with open('Data\\Mode_list.qes', 'r') as modes:
                ...
                for items in self.modes:  # assuming you need to create 5 buttons
                    self.button = Button(text=str(
                        items), background_color=(200, 200,200,1), background_normal = '', color='##2E4053')  # the text on the button
                ...
                self.add = Button(text='Add Mode', background_color=(
                    200, 200, 200, 1), background_normal='', color='##2E4053')
                self.add.bind(on_press= lambda x: add_mode())
                self.ids.add.add_widget(self.add)
